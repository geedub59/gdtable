  ...
  $.fn.gdtable = function (options) {
    ...
    var processFilter = debounce(function () {
      ...
      if (typeof settings.beforeFilter === "function") {
        settings.beforeFilter($gdtable);
      }
      $gdtable.applyFilter();
      if (typeof settings.afterFilter === "function") {
        settings.afterFilter($gdtable);
      }
    ...
    var recalcOnResize = function () {
      ...
      if ($gdtable.find("tbody:first").hasScrollBar().vertical) {
      ...
      sum = 0
      $gdtable.find("tr.gd-colWidths td").each(function (idx) {
        sum += $(this).outerWidth();
      });
      sum = Math.round(sum);
      // console.log('Total outerWidth = ' + sum);

      if (sum < containerWidth) {
        // set the column widths in the ratio of the last resized column action
        var maxItem = settings.colOuterWidth.length;
        var widthTotal = 0;
        settings.colOuterWidth.forEach(function (v, idx) {
          var i = idx + 1;
          var width = v / settings.colOuterWidthTotal * containerWidth;
          if (i == maxItem) {
            width = containerWidth - widthTotal;
          }
          widthTotal += width;
          ...
          $gdtable.find("thead tr.gd-colheadingrow:first th:nth-child(" + i + ")").css("width", width);
          $gdtable.find("tbody tr.gd-colWidths td:nth-child(" + i + ")").css("width", width);
        });
      } else {
        // set the width of all columns to be he same as the first body row
        $gdtable.find("tr.gd-colWidths td").each(function (idx) {
          var i = idx + 1;
          $gdtable.find("thead tr.gd-scrollbarrow:first th:nth-child(" + i + ")").css("width", $(this).css("width"));
          $gdtable.find("thead tr.gd-colfilterrow:first th:nth-child(" + i + ")").css("width", $(this).css("width"));
          $gdtable.find("thead tr.gd-colheadingrow:first th:nth-child(" + i + ")").css("width", $(this).css("width"));
        });
      }
    ...
    return this.each(function () {
      ...
      settings.colWidthRatio.unshift(.1);
  ...
  $.fn.gdtooltip = function (options) {
    ...
    if (settings.colToolTip.length > 0) {
      ...
      return this.each(function () {
        ...
        // for each row in the body
        $gdtable.find("tbody").find("tr").each(function () {
          // copy the <td> text to a data attribute and add the gdtooltip class
